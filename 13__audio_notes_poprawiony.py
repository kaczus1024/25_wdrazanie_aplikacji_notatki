...
def transcribe_audio(audio_bytes):
    ...
    # Przekazanie krotki (nazwa_pliku, dane_bajtkowe) chroni przed błędami MIME w OpenAI SDK
    audio_file = ("audio.mp3", audio_bytes)
...
def add_note_to_db(note_text):
    ...
    qdrant_client.upsert(
        ...
        points=[
            PointStruct(
                id=str(uuid.uuid4()),  # Użycie bezpiecznego identyfikatora UUID
...
def list_notes_from_db(query=None):
    ...
    if not query:
        ...
        # Zaktualizowana metoda query_points zamiast wycofanej search
...
with add_tab:
    ...
    # KLUCZOWA POPRAWKA: Sprawdzamy, czy długość nagrania jest większa od 0
    if note_audio and len(note_audio) > 0:
        audio = BytesIO()
